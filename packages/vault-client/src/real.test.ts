...
import { type BindingsVaultClient, RealVaultClient } from "./real";
...
function writeTx() {
  return {
    signAndSend: vi.fn(async (opts?: { signTransaction?: (xdr: string) => Promise<unknown> }) => {
...
describe("RealVaultClient reads", () => {
  ...
  it("poolStatus maps the contract tag to the seam union", async () => {
    ...
    const frozen = new RealVaultClient({
      ...opts,
      client: makeClient({
        pool_status: vi.fn(() => readTx({ tag: "Frozen", values: undefined })),
      }),
    });
