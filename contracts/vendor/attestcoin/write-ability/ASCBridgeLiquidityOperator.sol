...
contract ASCBridgeLiquidityOperator is IASCBridgeLiquidityOperator, Ownable2Step {
    ...
    event ChainConfigSet(bytes32 indexed chainKey, address indexed outbox, bool enabled);
    event BridgeMessagePublished(
        bytes32 indexed intentId, bytes32 indexed messageId, bytes32 indexed chainKey
    );
    ...
    event OutboundFeePayerRecorded(
        bytes32 indexed messageId, address indexed payer, address indexed relayer
    );
    event RelayerRefundForwarded(bytes32 indexed messageId, address indexed payer, uint256 amount);
    event CoreFeeRefundForwarded(bytes32 indexed messageId, address indexed payer, uint256 amount);
    event AckFeeRefundForwarded(bytes32 indexed messageId, address indexed payer, uint256 amount);
    event ChainConfigRemoved(bytes32 indexed chainKey);
    event ChainRelayerSet(bytes32 indexed chainKey, address indexed relayerContract);
    event SourceEvmChainIdSet(bytes32 indexed chainKey, uint256 indexed sourceEvmChainId);
    ...
    event RemoteClientOperatorSet(bytes32 indexed chainKey, address indexed clientOperator);
    event BridgeIntentExecutionTracked(
        bytes32 indexed intentId, bool success, uint256 gasUsed, bytes32 returnedDataHash
    );
    ...
    function outboundTokenRoutes(bytes32 chainKey)
        external
        view
        returns (
            address destinationToken,
    ...
    function setChainConfig(bytes32 chainKey, address outbox, bool enabled) external onlyOwner {
        if (outbox == address(0)) {
            revert CommonErrors.ZeroAddress();
        }
        ...
        if (
            address(existing.outbox) != address(0) && address(existing.outbox) != outbox
                && address(existing.relayerContract) != address(0)
        ) {
            existing.outbox.approveForwarder(address(existing.relayerContract), false);
        }
        ...
        _chainConfigs[chainKey] = ChainConfig({
            outbox: IOutbox(outbox), enabled: enabled, relayerContract: configuredRelayer
        });
    ...
    function setChainRelayer(bytes32 chainKey, address relayerContract_) external onlyOwner {
        ChainConfig storage config = _chainConfigs[chainKey];
        if (address(config.outbox) == address(0)) {
            revert ASCBridgeLiquidityOperatorErrors.ChainKeyNotConfigured(chainKey);
        }
        ...
        if (actualOutbox != address(config.outbox)) {
            revert ASCBridgeLiquidityOperatorErrors.RelayerOutboxMismatch(
                address(config.outbox), actualOutbox
            );
        }
    ...
    function setRemoteClientOperator(bytes32 chainKey, address clientOperator) external onlyOwner {
    ...
    function removeChainConfig(bytes32 chainKey) external onlyOwner {
        ...
        if (address(config.outbox) != address(0) && address(config.relayerContract) != address(0)) {
            config.outbox.approveForwarder(address(config.relayerContract), false);
        }
    ...
    function setSourceEvmChainId(bytes32 chainKey, uint256 sourceEvmChainId) external onlyOwner {
    ...
    function setInboundTokenRoute(
        ...
        if (destinationDecimals != actualDestinationDecimals) {
            revert ASCBridgeLiquidityOperatorErrors.DestinationDecimalsMismatch(
                actualDestinationDecimals, destinationDecimals
            );
        }
        ...
        if (route.sourceToken != address(0)) {
            if (
                route.sourceToken != sourceToken || route.sourceDecimals != sourceDecimals
                    || route.destinationDecimals != destinationDecimals
            ) {
                revert ASCBridgeLiquidityOperatorErrors.InboundTokenRouteImmutable(chainKey);
            }
        ...
        emit InboundTokenRouteSet(
            chainKey, sourceToken, sourceDecimals, destinationDecimals, enabled
        );
    }
    ...
    function setOutboundTokenRoute(
        ...
        if (route.minimumGasLimit != 0) {
            if (
                route.destinationToken != destinationToken || route.sourceDecimals != sourceDecimals
                    || route.destinationDecimals != destinationDecimals
            ) {
                revert ASCBridgeLiquidityOperatorErrors.OutboundTokenRouteImmutable(chainKey);
            }
            route.enabled = enabled;
            ...
            if (sourceDecimals != actualSourceDecimals) {
                revert ASCBridgeLiquidityOperatorErrors.SourceDecimalsMismatch(
                    actualSourceDecimals, sourceDecimals
                );
            }
    ...
    function getChainConfig(bytes32 chainKey) external view returns (address outbox, bool enabled) {
        ChainConfig memory config = _chainConfigs[chainKey];
        return (address(config.outbox), config.enabled);
    }

    function getChainRelayer(bytes32 chainKey) external view returns (address relayerContract_) {
    ...
    function bridgeTo(
        ...
        if (msg.value != 0) {
            revert ASCBridgeLiquidityOperatorErrors.NativeValueUnsupported(msg.value);
        }
        ChainConfig memory config = _chainConfigs[chainKey];
        uint32 outboxChainKey = _requireOutboundPublishReady(chainKey, config);
        (bool hasTokenTransfer, ASCBridgeTypes.BridgeMessage memory outboundMessage) =
            _validatedOutboundMessage(chainKey, message);

        (bytes memory signedQuote, uint256 tip, uint256 tipExpiry) =
            abi.decode(quote, (bytes, uint256, uint256));
        RelayerTypes.Quote memory q = abi.decode(signedQuote, (RelayerTypes.Quote));
        if (q.payInNative) {
        ...
        if (q.destinationChain != outboxChainKey) {
            revert ASCBridgeLiquidityOperatorErrors.QuoteDestinationMismatch(
                outboxChainKey, q.destinationChain
            );
        }
        if (q.gasLimit < outboundMessage.gasLimit) {
            revert ASCBridgeLiquidityOperatorErrors.QuoteGasLimitTooLow(
                q.gasLimit, outboundMessage.gasLimit
            );
        }

        uint256 nonce = intentNonces[msg.sender];
        (bytes32 intentId, bytes memory encodedPayload) =
            _outboundPayload(msg.sender, chainKey, outboundMessage, nonce, hasTokenTransfer);
        ...
        if (hasTokenTransfer) {
            token.compatibleTransferFrom(
                msg.sender, address(this), outboundMessage.tokenAmount.amount
            );
            if (!_usesCreditcoinEscrow()) {
                IERC20Burnable(address(token)).burn(outboundMessage.tokenAmount.amount);
            }
        }
        ...
        IERC20 attest = IERC20(IRelayerAttestToken(address(config.relayerContract)).attestToken());
        ...
        bytes32 messageId = config.relayerContract
            .publishAndCollectRelayerFee(encodedPayload, signedQuote, tip, tipExpiry);
        // Redirect claimDelivery / deadline tip+top-up refunds to the user.
        // Older Relayers without this API are skipped (try/catch).
        try config.relayerContract.setFeeRefundRecipient(messageId, msg.sender) {} catch {}
    ...
    function _snapshotAttestorVault(IOutbox publishedOutbox) internal view returns (address vault) {
    ...
    function requestRelayerRefund(bytes32 messageId) external override {
        (IRelayerContract relayer, address payer, IERC20 attest) =
            _requireOutboundFeePayer(messageId);
        if (msg.sender != payer) {
            revert ASCBridgeLiquidityOperatorErrors.NotOutboundFeePayer(msg.sender, payer);
        }
        OutboundFeeRecord storage record = outboundFeeRecords[messageId];
        if (record.relayerRefunded) {
            revert ASCBridgeLiquidityOperatorErrors.OutboundFeeComponentAlreadyRefunded(messageId);
        }
    ...
    function increaseRelayerTip(bytes32 messageId, uint256 additionalTip, uint256 newTipExpiry)
        external
        override
    {
        if (additionalTip == 0) {
    ...
    function previewBridgePayload(
        ...
        view
        override
        returns (uint256 nonce, bytes32 intentId, bytes memory payload, bytes32 payloadHash)
    {
        if (sender == address(0)) {
            revert CommonErrors.ZeroAddress();
        }
        ...
        (intentId, payload) =
            _outboundPayload(sender, chainKey, outboundMessage, nonce, hasTokenTransfer);
    ...
    function _validatedOutboundMessage(
        ...
        internal
        view
        returns (bool hasTokenTransfer, ASCBridgeTypes.BridgeMessage memory outboundMessage)
    {
        ...
        uint256 destinationAmount =
            _normalizedOutboundAmount(chainKey, message.tokenAmount.amount, route);
        _validateOutboundTokenCallData(message.data, receiver, destinationAmount);
        bytes memory executionCommitment = abi.encode(
            _outboundTokenExecutionCommitment(message, receiver, destinationAmount, route)
        );
    ...
    function _outboundPayload(
        ...
        uint8 msgType = _deriveOutboundMsgType(hasTokenTransfer);
        intentId = keccak256(abi.encodePacked(msgType, nonce, chainKey, sender));
        BridgeMessageCodecV1.BridgePayloadV1 memory payload =
            BridgeMessageCodecV1.BridgePayloadV1({intentId: intentId, message: message});
        encodedPayload = BridgeMessageCodecV1.encode(payload);
    }

    function _requireOutboundFeePayer(bytes32 messageId)
        internal
        view
        returns (IRelayerContract relayer, address payer, IERC20 attest)
    {
        ...
        if (record.payer == address(0) || record.relayer == address(0)) {
            revert ASCBridgeLiquidityOperatorErrors.UnknownOutboundMessage(messageId);
        }
    ...
    function _requireOutboundPublishReady(bytes32 chainKey, ChainConfig memory config)
        internal
        view
        returns (uint32 outboxChainKey)
    {
        if (address(config.outbox) == address(0)) {
            revert ASCBridgeLiquidityOperatorErrors.ChainKeyNotConfigured(chainKey);
        }
        ...
        if (address(config.relayerContract) == address(0)) {
            revert ASCBridgeLiquidityOperatorErrors.ChainRelayerNotConfigured(chainKey);
        }
    ...
    function _requireOutboxChainKey(bytes32 chainKey, IOutbox outbox)
        internal
        view
        returns (uint32 expected)
    {
        if (uint256(chainKey) > type(uint32).max) {
            revert ASCBridgeLiquidityOperatorErrors.InvalidChainKeyEncoding(chainKey);
        }
        expected = uint32(uint256(chainKey));
        uint32 actual = outbox.chainKey();
        if (actual != expected) {
            revert ASCBridgeLiquidityOperatorErrors.OutboxChainKeyMismatch(expected, actual);
        }
    }

    function _requireRelayerDestination(IRelayerContract relayer, uint32 destinationChain)
        internal
        view
    {
        if (relayer.destinationEvmChainIds(destinationChain) == 0) {
            revert ASCBridgeLiquidityOperatorErrors.RelayerDestinationNotConfigured(destinationChain);
        }
    }
    ...
    function bridgeFromIntent(
    ...
    ) external override returns (bool isValid, bytes[] memory extractedTransactionData) {
        _validateInboundInputs(chainKey, blockHeight);
        bytes memory encodedTransaction =
            _verifyProofs(chainKey, blockHeight, inclusionProof, continuityProof);

        IBridgeIntentDecoder.DecodedBridgeIntent memory decoded =
            bridgeIntentDecoder.decodeBridgeIntent(encodedTransaction);
        CrossChainOrderTypes.CrossChainOrder memory order = decoded.order;
        ASCBridgeTypes.AttestedTxData memory decodedAttestedTxData = decoded.attestedTxData;

        bytes32 intentId = keccak256(abi.encodePacked(chainKey, order.nonce, order.user));
        ...
        bytes memory destinationCallData =
            _destinationCallData(intentId, intent.recipient, destinationAmount);
        extractedTransactionData =
            _buildExtractedTransactionData(decodedAttestedTxData, intent, intentId);
        ...
        if (!_validateInboundIntentAndMatchAttestation(
                chainKey,
                order,
                intent,
        ...
        isValid = _processValidatedIntent(chainKey, intentId, order, intent, destinationCallData);
        return (isValid, extractedTransactionData);
    }

    function _hasTokenTransfer(ASCBridgeTypes.EVMTokenAmount calldata evmTokenAmount)
        internal
        view
        returns (bool)
    {
        ...
        if (evmTokenAmount.token != address(token)) {
            revert ASCBridgeLiquidityOperatorErrors.InvalidTokenAddress(evmTokenAmount.token);
        }
        if (evmTokenAmount.amount == 0) {
            revert ASCBridgeLiquidityOperatorErrors.InvalidTokenAmount();
        }
    ...
    function _validateInboundInputs(bytes32 chainKey, uint64 blockHeight) internal view {
        ChainConfig memory config = _chainConfigs[chainKey];
        if (address(config.outbox) == address(0)) {
            revert ASCBridgeLiquidityOperatorErrors.ChainKeyNotConfigured(chainKey);
        }
        ...
        if (sourceEvmChainIds[chainKey] == 0) {
            revert ASCBridgeLiquidityOperatorErrors.SourceEvmChainIdNotConfigured(chainKey);
        }
        if (blockHeight == 0) {
            revert ASCBridgeLiquidityOperatorErrors.InvalidBlockHeight();
        }
        if (address(proofVerifier) == address(0) || address(bridgeIntentDecoder) == address(0)) {
    ...
    function _verifyProofs(
        ...
        return proofVerifier.verifyProofs(chainKey, blockHeight, inclusionProof, continuityProof);
    }

    function _callDestination(address receiver, bytes memory callData)
        internal
    ...
    function _validateInboundIntentAndMatchAttestation(
        ...
        if (
            order.openDeadline != 0 && order.fillDeadline != 0
        ...
        if (
            !amountConvertible || destinationAmount == 0
                || intent.minDestinationAmount.amount > destinationAmount
        ) {
            return false;
        }
        address expectedDestination = _usesCreditcoinEscrow() ? address(token) : mintDestination;
        if (intent.recipient == address(0) || intent.destinationContract != expectedDestination) {
            return false;
        }
        if (keccak256(intent.destinationCallData) != keccak256(destinationCallData)) {
            return false;
        }
        return true;
    }
    ...
    function _normalizedInboundAmount(
        ...
        if (
            !route.enabled || route.sourceToken == address(0)
                || attestedTxData.sourceAmount.token != route.sourceToken
        ) {
            return (false, 0);
        }
        return TokenAmountNormalization.tryNormalize(
            attestedTxData.sourceAmount.amount, route.sourceDecimals, route.destinationDecimals
        );
    }
    ...
    function _normalizedOutboundAmount(
        ...
        if (!route.enabled) {
            revert ASCBridgeLiquidityOperatorErrors.OutboundTokenRouteNotConfigured(chainKey);
        }
        if (amount == 0) {
            revert ASCBridgeLiquidityOperatorErrors.InvalidTokenAmount();
        }

        (bool ok, uint256 normalizedAmount) = TokenAmountNormalization.tryNormalize(
            amount, route.sourceDecimals, route.destinationDecimals
        );
        if (!ok) {
    ...
    function _validateOutboundTokenCallData(
        ...
        bytes memory expectedCallData =
            CanonicalTokenCall.encode(_destinationUsesMint(), receiver, destinationAmount);
        if (
    ...
    function _outboundTokenExecutionCommitment(
        ...
        return TokenExecutionCommitment.compute(
            block.chainid,
    ...
    function _validateOutboundGasLimit(uint256 gasLimit, uint256 minimumGasLimit) internal pure {
    ...
    function _validateOutboundReceiver(bytes calldata encodedReceiver)
        internal
        pure
        returns (address receiver)
    {
        ...
        if (encodedReceiver.length != 32) {
            revert ASCBridgeLiquidityOperatorErrors.InvalidEvmReceiverLength(encodedReceiver.length);
        }
    ...
    function _processValidatedIntent(
        ...
        address destination = _usesCreditcoinEscrow() ? address(token) : mintDestination;
        (bool success, bytes memory retData, uint256 gasUsed) =
            _callDestination(destination, destinationCallData);
        if (success && _usesCreditcoinEscrow() && !CompatibleERC20.isSuccessfulReturn(retData)) {
            revert CompatibleERC20.ERC20CallFailed(address(token), IERC20.transfer.selector);
        }
        _intentStatus[intentId] = _INTENT_NONE;
        emit BridgeIntentExecutionTracked(intentId, success, gasUsed, keccak256(retData));
        if (!success) {
            return false;
        }
        uint256 actualGasCost = gasUsed * tx.gasprice;
        if (intent.maxGasCost != 0 && actualGasCost > intent.maxGasCost) {
            revert ASCBridgeLiquidityOperatorErrors.MaxGasCostExceeded(
                intent.maxGasCost, actualGasCost
            );
        }
    ...
    function _destinationCallData(bytes32 intentId, address recipient, uint256 amount)
        internal
        view
        returns (bytes memory)
    {
        ...
        return abi.encodeCall(
            IASCBridgeTokenDestination.executeMint, (intentId, address(token), recipient, amount)
        );
    }

    function _deriveOutboundMsgType(bool hasTokenTransfer) internal pure returns (uint8 msgType) {
        if (hasTokenTransfer) {
    ...
    function _requireCompatibleMintDestination(address destination) internal view {
        if (destination.code.length == 0) {
            revert ASCBridgeLiquidityOperatorErrors.InvalidMintDestination(destination);
        }

        address configuredOperator;
        try IASCBridgeTokenDestination(destination).bridgeOperator() returns (address operator) {
        ...
        if (configuredOperator != address(this)) {
            revert ASCBridgeLiquidityOperatorErrors.MintDestinationOperatorMismatch(
                address(this), configuredOperator
            );
        }

        bool tokenSupported;
        try IASCBridgeTokenDestination(destination).mintTokenAllowed(address(token)) returns (
            bool supported
        ) {
        ...
        if (!tokenSupported) {
            revert ASCBridgeLiquidityOperatorErrors.MintDestinationTokenUnsupported(address(token));
        }
    }

    function _compatibleApprove(IERC20 token_, address spender, uint256 amount) private {
        (bool success, bytes memory returnData) =
            address(token_).call(abi.encodeCall(IERC20.approve, (spender, amount)));
        if (!success || !CompatibleERC20.isSuccessfulReturn(returnData)) {
            revert CompatibleERC20.ERC20CallFailed(address(token_), IERC20.approve.selector);
        }
    }
}
