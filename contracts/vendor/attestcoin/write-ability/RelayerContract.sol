...
contract RelayerContract is IRelayerContract, RelayerFeeLedger, Ownable2Step {
    ...
    bytes32 private constant _PUBLISHER_AUTHORIZATION_DOMAIN =
        keccak256("RelayerContract.publisherFeeAuthorization.v1");
    ...
    constructor(
        ...
        if (
            attestToken_ == address(0) || outbox_ == address(0) || quoterContract_ == address(0)
                || proofVerifier_ == address(0) || deliveryDecoder_ == address(0)
    ...
    function setDestinationEvmChainId(uint32 destinationChain, uint32 destinationEvmChainId)
        external
        override
        onlyOwner
    {
    ...
    function setTrustedPublisher(address publisher, bool trusted) external override onlyOwner {
    ...
    function publisherAuthorizationNonce(address payer, address publisher, bytes32 quoteDigest)
        public
        view
        override
        returns (bytes32)
    ...
    function publishAndCollectRelayerFee(
        ...
        (RelayerTypes.Quote memory q, uint256 coreFee, bytes32 quoteDigest) =
    ...
    function publishAndCollectRelayerFeeWithAuthorization(
        ...
        IERC3009(address(attestToken))
            .receiveWithAuthorization(
                msg.sender, address(this), total, validAfter, validBefore, nonce, v, r, s
            );
    ...
    function publishAndCollectRelayerFeeFor(
        ...
        (bytes memory signedQuote, bytes memory authorization) =
            abi.decode(feeAuthorization, (bytes, bytes));
        ...
        (uint256 validAfter, uint256 validBefore, bytes32 nonce, uint8 v, bytes32 r, bytes32 s) =
            abi.decode(authorization, (uint256, uint256, bytes32, uint8, bytes32, bytes32));
        bytes32 expectedNonce = publisherAuthorizationNonce(payer, msg.sender, quoteDigest);
        if (nonce != expectedNonce) {
            revert RelayerErrors.PublisherAuthorizationNonceMismatch(expectedNonce, nonce);
        }

        IERC3009(address(attestToken))
            .receiveWithAuthorization(
                payer, address(this), quotedTotal, validAfter, validBefore, nonce, v, r, s
            );
    ...
    function collectRelayerFee(
        ...
        if (q.payInNative) {
            ...
            if (q.acknowledgmentPrice > 0) {
                attestToken.compatibleTransferFrom(msg.sender, address(this), q.acknowledgmentPrice);
            }
        } else {
            if (msg.value != 0) {
                revert RelayerErrors.InvalidNativeAmount(0, msg.value);
    ...
    function claimDelivery(
        ...
        bytes memory encodedTx =
            proofVerifier.verifyProofs(chainKey, blockHeight, inclusionProof, continuityProof);
        ...
        (, uint256 relayFeePaid, uint256 unusedRefunded, uint256 tipPaid, uint256 tipRefunded) =
    ...
    function requestTopUp(bytes32 messageId, uint256 additionalGasNeeded) external override {
        ...
        if (block.timestamp >= info.deliveryDeadline) {
            revert RelayerErrors.DeliveryDeadlineReached(
                messageId, info.deliveryDeadline, block.timestamp
            );
        }
    ...
    function topUpGasLimit(
        ...
        RelayerTypes.TopUpQuote memory q = _checkTopUp(
            messageId, signedTopUpQuote, additionalATTEST
        );
        _collectDelta(messageId, additionalATTEST);
        uint256 oldGasLimit = _applyTopUp(messageId, q.newGasLimit, additionalATTEST);
    ...
    function topUpGasLimitWithAuthorization(
        ...
        IERC3009(address(attestToken))
            .receiveWithAuthorization(
                msg.sender, address(this), additionalATTEST, validAfter, validBefore, nonce, v, r, s
            );
    ...
    function setFeeRefundRecipient(bytes32 messageId, address recipient) external override {
    ...
    function _collectDelta(bytes32 messageId, uint256 amount) internal {
        if (_feesInNative(messageId)) {
            ...
            attestToken.compatibleTransferFrom(msg.sender, address(_vaultFor(messageId)), amount);
        }
    }
    ...
    function increaseTip(bytes32 messageId, uint256 additionalTip, uint256 newTipExpiry)
        external
        payable
        override
    {
    ...
    function increaseTipWithAuthorization(
        ...
        if (_feesInNative(messageId)) {
            revert RelayerErrors.NativePaymentNotSupported();
        }
        _validateTipIncrease(messageId, additionalTip, newTipExpiry);
        IERC3009(address(attestToken))
            .receiveWithAuthorization(
                msg.sender, address(this), additionalTip, validAfter, validBefore, nonce, v, r, s
            );
    ...
    function _validateTipIncrease(bytes32 messageId, uint256 additionalTip, uint256 newTipExpiry)
        internal
        view
    {
        RelayerTypes.MessageInfo memory info = getMessageInfo(messageId);
        ...
        if (newTipExpiry <= info.deliveryDeadline) {
            revert RelayerErrors.TipExpiryNotAfterDeliveryDeadline(
                messageId, info.deliveryDeadline, newTipExpiry
            );
        }
    }
        ...
        returns (RelayerTypes.Quote memory q, uint256 coreFee, bytes32 digest)
    {
        ...
        if (q.expectedCompletion <= block.timestamp) {
            revert RelayerErrors.InvalidDeliveryDeadline(q.expectedCompletion, block.timestamp);
        }
        digest = keccak256(
            abi.encode(
        ...
        if (q.destinationChain != outboxChain) {
            revert RelayerErrors.QuoteDestinationChainMismatch(outboxChain, q.destinationChain);
        }
        ...
        uint256 relayFeeFloor = q.payInNative
            ? quoterContract.requestQuote(
                uint16(q.destinationChain), q.targetContract, q.payloadHash, q.gasLimit
            )
            : quoterContract.requestQuoteInAttest(
                uint16(q.destinationChain), q.targetContract, q.payloadHash, q.gasLimit
            );
        if (q.relayPrice < relayFeeFloor) {
            revert RelayerErrors.RelayFeeBelowFloor(q.relayPrice, relayFeeFloor);
        }
        ...
        if (q.acknowledgmentPrice != 0) {
            uint256 ackFeeFloor = quoterContract.getAcknowledgmentFee(uint16(q.destinationChain));
            if (q.acknowledgmentPrice < ackFeeFloor) {
                revert RelayerErrors.AcknowledgmentFeeBelowFloor(q.acknowledgmentPrice, ackFeeFloor);
            }
        }
    }
    ...
    function _validateStoredMessageQuote(
        ...
        if (message.payloadHash != q.payloadHash) {
            revert RelayerErrors.QuotePayloadHashMismatch(q.payloadHash, message.payloadHash);
        }
    ...
    function _depositRelay(
        ...
        if (tip > 0 && tipExpiry <= q.expectedCompletion) {
            revert RelayerErrors.TipExpiryNotAfterDeliveryDeadline(
                messageId, q.expectedCompletion, tipExpiry
            );
        }
        uint32 destinationEvmChainId = destinationEvmChainIds[q.destinationChain];
        if (destinationEvmChainId == 0) {
            revert RelayerErrors.DestinationEvmChainIdNotConfigured(q.destinationChain);
        }
