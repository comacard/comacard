...
contract OutboxDeployer is Ownable2Step {
    ...
    error ChainNotRegistered(uint32 chainKey);
    ...
    function enableFactory(address factory) external onlyOwner {
        if (!factoryEnabled[factory] && factories[IOutboxFactory(factory).version()] != factory) {
    ...
    function registerChain(uint32 chainKey, uint256 chainId) external onlyOwner {
    ...
    function deployOutbox(
        ...
        if (chainIdOf[chainKey] == 0) revert ChainNotRegistered(chainKey);

        outbox = IOutboxFactory(factory)
            .deployOutbox(
                chainKey,
                outboxOwner,
                validator,
        ...
        emit OutboxDeployed(chainKey, chainIdOf[chainKey], outbox, factory, version);
    }
}
