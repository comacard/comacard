...
import {QueryProofVerificationLib} from "./common/QueryProofVerificationLib.sol";
...
contract AcknowledgmentValidator is Ownable2Step {
    ...
    constructor(
        ...
        if (_proofVerifier == address(0) || _attestToken == address(0)) {
            revert ZeroAddress();
        }
    ...
    function updateTrustedInbox(address _inbox, bool _trusted) external onlyOwner {
    ...
    function submitAcknowledgment(
        ...
        uint256 txSize = QueryProofVerificationLib.txBytesFromInclusion(inclusionProof).length;
        if (txSize > MAX_ENCODED_TRANSACTION_BYTES) {
            revert EncodedTransactionTooLarge(txSize, MAX_ENCODED_TRANSACTION_BYTES);
        }
        ...
        bytes memory encodedTransaction = proofVerifier.verifyProofs(
            bytes32(uint256(destinationChainKey)), height, inclusionProof, continuityProof
        );
