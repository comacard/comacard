...
contract TWAPReader is ITWAPReader, Ownable2Step {
    ...
    event PriceObserved(
        uint256 indexed spotPrice, uint256 indexed cumulative, uint32 indexed timestamp
    );
    ...
    function update(uint256 spotPrice) external onlyOracle {
        ...
        // observations.
        uint256 newIndex =
            overwriteLatest ? obsIndex : obsCount == 0 ? 0 : (obsIndex + 1) % BUFFER_SIZE;
    ...
    function read() external view override returns (uint256 ctcPerAttest) {
        ...
        if (oldest.timestamp > target) {
            revert RelayerErrors.InsufficientPoolHistory(nowTs - oldest.timestamp, window);
        }
    ...
    function _cumulativeAt(uint32 target) private view returns (uint256) {
        ...
        for (uint256 i = 0; i < count; ++i) {
            ...
            if (obs.timestamp < target) {
                uint256 segmentCumulative = afterObservation.cumulative - obs.cumulative;
                uint32 segmentSeconds = afterObservation.timestamp - obs.timestamp;
                return obs.cumulative
                    + FullMath.mulDiv(segmentCumulative, target - obs.timestamp, segmentSeconds);
            }
    ...
    function _oldestObservation() private view returns (Observation memory) {
        uint256 oldestIndex = (obsIndex + BUFFER_SIZE + 1 - obsCount) % BUFFER_SIZE;
