...
contract EVMDeliveryDecoder is IDeliveryDecoder, Ownable2Step {
    ...
    uint256 private constant _DELIVER_MESSAGE_MIN_CALLDATA_LENGTH = 4 + 4 * 32;
    // Exact event signature text, required for the topic hash.
    // solhint-disable-next-line gas-small-strings
    bytes32 private constant _DELIVERED_EVENT_SIGNATURE =
        keccak256("MessageDelivered(bytes32,address,address)");
    bytes32 private constant _PENDING_EVENT_SIGNATURE =
        keccak256("MessagePending(bytes32,address,address)");

    mapping(uint32 => address) public trustedInboxes;

    event TrustedInboxSet(uint32 indexed destinationChainId, address indexed inbox);
    ...
    function setTrustedInbox(uint32 destinationChainId, address inbox) external onlyOwner {
    ...
    function decodeDelivery(bytes calldata encodedTransaction)
        external
        view
        override
        returns (AttestedDeliveryData memory decoded)
    {
        ASCSdkV1TxBytesLib.ProofTx memory proofTx = ASCSdkV1TxBytesLib.decode(encodedTransaction);
        if (proofTx.chainId == 0 || proofTx.chainId > type(uint32).max) {
        ...
        if (proofTx.toIsNull || trustedInbox == address(0) || trustedInbox != proofTx.to) {
        ...
        for (uint256 i; i < receipt.receiptLogs.length; ++i) {
            ...
            if (
                logEntry.address_ != proofTx.to || logEntry.topics.length < 2
            ...
            if (logEntry.topics[0] == _DELIVERED_EVENT_SIGNATURE && logEntry.topics.length == 4) {
                status = ExecutionStatus.Success;
            } else if (
                logEntry.topics[0] == _PENDING_EVENT_SIGNATURE && logEntry.topics.length == 4
            ) {
    ...
    function _decodeMessageId(bytes memory transactionData)
        internal
        pure
        returns (bytes32 messageId)
    {
        if (transactionData.length < _DELIVER_MESSAGE_MIN_CALLDATA_LENGTH) {
            revert UnsupportedDestinationTransaction();
        }
