...
library CompatibleERC20 {
    ...
    function compatibleTransfer(IERC20 token, address to, uint256 amount) internal {
        _call(address(token), abi.encodeCall(IERC20.transfer, (to, amount)));
    }

    function compatibleTransferFrom(IERC20 token, address from, address to, uint256 amount)
        internal
    {
        _call(address(token), abi.encodeCall(IERC20.transferFrom, (from, to, amount)));
    }

    function compatibleMint(IERC20Mintable token, address to, uint256 amount) internal {
        _call(address(token), abi.encodeCall(IERC20Mintable.mint, (to, amount)));
    }

    function isSuccessfulReturn(bytes memory returnData) internal pure returns (bool) {
        return isSuccessfulReturn(returnData, returnData.length);
    }

    function isSuccessfulReturn(bytes memory returnData, uint256 fullReturnDataSize)
        internal
        pure
        returns (bool)
    {
        ...
        return fullReturnDataSize == 32 && returnData.length == 32 && _returnedTrue(returnData);
    }
