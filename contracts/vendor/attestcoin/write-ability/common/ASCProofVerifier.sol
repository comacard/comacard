...
contract ASCProofVerifier is IASCProofVerifier {
    ...
    function calculateTxIndex(BlockProverTypes.InclusionProof calldata inclusionProof)
        external
        view
        returns (uint64)
    {
    ...
    function _verifyBinaryMerkle(
        ...
        (txBytes, siblings) =
            QueryProofVerificationLib.decodeBinaryMerklePayload(inclusionProof.data);
        if (txBytes.length == 0) {
            revert ProofInvalid(chainKey, blockHeight);
        }
    ...
    function _verifyViaNativeQueryPrecompile(
        ...
        INativeQueryVerifier.ContinuityProof memory nativeContinuity =
        ...
        return NativeQueryVerifierLib.getVerifier()
            .verifyAndEmit(
                uint64(uint256(chainKey)), blockHeight, txBytes, merkleProof, nativeContinuity
            );
    }

    function _nativeMerkleProof(BlockProverTypes.InclusionProof calldata inclusionProof)
        internal
        pure
    ...
    function _toNativeMerkleProof(bytes32 root, BlockProverTypes.MerkleProofEntry[] memory siblings)
        internal
        pure
        returns (INativeQueryVerifier.MerkleProof memory merkleProof)
    {
        ...
        for (uint256 i = 0; i < length; ++i) {
            nativeSiblings[i] = INativeQueryVerifier.MerkleProofEntry({
                hash: siblings[i].sibling, isLeft: siblings[i].isLeft
            });
        }
        merkleProof = INativeQueryVerifier.MerkleProof({root: root, siblings: nativeSiblings});
    }
}
