...
contract BridgeIntentDecoder is BridgeIntentTemplateRegistry, IBridgeIntentDecoder {
    ...
    mapping(uint64 => mapping(address => SourceSettlementPolicy)) public sourceSettlementPolicies;

    event TrustedSourceSettlerSet(
        uint64 indexed sourceChainId, address indexed settler, bool trusted
    );
    ...
    error SettlementRecipientNotTrustedSettler(address settler, address settlementRecipient);
    error SettlementTransferNotProved(address token, address from, address to, uint256 amount);
    ...
    function setTrustedSourceSettler(uint64 sourceChainId, address settler, bool trusted)
        external
        onlyOwner
    {
    ...
    function setSourceSettlementPolicy(
        ...
        sourceSettlementPolicies[sourceChainId][settler] = SourceSettlementPolicy({
            sourceToken: sourceToken, settlementRecipient: settlementRecipient, trusted: trusted
        });
        trustedSourceSettlers[sourceChainId][settler] = trusted;

        emit SourceSettlementPolicySet(
            sourceChainId, settler, sourceToken, settlementRecipient, trusted
        );
        emit TrustedSourceSettlerSet(sourceChainId, settler, trusted);
    }

    /// @inheritdoc IBridgeIntentDecoder
    function decodeBridgeIntent(bytes calldata encodedTransaction)
        external
        view
        returns (DecodedBridgeIntent memory)
    {
    ...
    function decodeFromTxBytes(bytes calldata txBytes)
        external
        view
        returns (DecodedBridgeIntent memory)
    {
    ...
    function decodeCrossChainIntentSourceAmount(bytes calldata orderData)
        external
        pure
        returns (ASCBridgeTypes.EVMTokenAmount memory)
    {
        return IntentTemplateBase.crossChainIntentSourceAmount(orderData);
    }

    function _decodeTxBytes(bytes calldata txBytes, bool requireAuthorizedSettlement)
        internal
        view
        returns (DecodedBridgeIntent memory decoded)
    {
        ...
        SourceSettlementPolicy memory policy = sourceSettlementPolicies[proofTx.chainId][proofTx.to];
        if (
        ...
        if (orderExtract.ok) {
            ...
            if (orderExtract.encodedIntent.length != 0) {
                intent =
                    abi.decode(orderExtract.encodedIntent, (CrossChainOrderTypes.CrossChainIntent));
        ...
        if (requireAuthorizedSettlement) {
            ...
            _requireTrustedSettlementRecipient(proofTx.to, policy.settlementRecipient);
            ...
            return DecodedBridgeIntent({
                order: order,
                intent: intent,
                attestedTxData: _attestedFromOrder(txBytes, order, sourceAmount),
    ...
    function resolveSourceAmountFromOrderData(
        ...
        (bool templateOk, ASCBridgeTypes.EVMTokenAmount memory templateAmount) = IntentTemplateBase.tryResolveSourceAmountFromOrderData(
            BridgeIntentTemplateRegistry(address(this)), _orderDataTemplateNames, proofTx, orderData
        );
        if (templateOk) {
    ...
    function _requireTrustedSettlementRecipient(address settler, address settlementRecipient)
        internal
        pure
    {
        if (settlementRecipient != address(0) && settlementRecipient != settler) {
            revert SettlementRecipientNotTrustedSettler(settler, settlementRecipient);
        }
    }
    ...
    function _requireSettlementTransfer(
        ...
        for (uint256 i; i < receipt.receiptLogs.length; ++i) {
            ...
            if (
                logEntry.address_ == sourceToken && logEntry.topics.length == 3
                    && logEntry.topics[0] == _TRANSFER_EVENT_SIGNATURE
                    && address(uint160(uint256(logEntry.topics[1]))) == user
                    && address(uint160(uint256(logEntry.topics[2]))) == settlementRecipient
                    && logEntry.data.length == 32 && abi.decode(logEntry.data, (uint256)) == amount
            ) {
                return;
            }
        }

        revert SettlementTransferNotProved(sourceToken, user, settlementRecipient, amount);
    }
