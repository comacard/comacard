...
library ASCSdkV1TxBytesLib {
    ...
    function _decodeMiddleFields(uint8 txType, bytes[] memory chunks, ProofTx memory proofTx)
        internal
        pure
    {
        if (txType == 0) {
            (uint128 gasPrice, uint256 v, bytes32 unused1, bytes32 unused2) =
                abi.decode(chunks[1], (uint128, uint256, bytes32, bytes32));
            unused1;
            unused2;
        ...
        if (txType == 1) {
            (
                ...
                bytes32 r,
                bytes32 s
            ) = abi.decode(chunks[1], (uint64, uint128, AccessListEntry[], uint8, bytes32, bytes32));
            accessList;
            yParity;
            r;
            s;
        ...
        if (txType == 2) {
            (
                ...
                chunks[1], (uint64, uint128, uint128, AccessListEntry[], uint8, bytes32, bytes32)
            );
            accessList;
            yParity;
            r;
