...
library RateLimitLib {
    ...
    function enforce(RateBucket storage bucket, uint128 policy) internal {
        ...
        // Compare elapsed time instead of adding the caller-configured window
        // to a live timestamp. The addition can overflow uint64 for otherwise
        // valid encoded policies and panic every publication attempt.
        bool windowExpired = nowTs >= bucket.windowStart && nowTs - bucket.windowStart >= window;
        if (bucket.windowStart == 0 || windowExpired) {
            bucket.windowStart = nowTs;
            bucket.used = 0;
        }

        if (bucket.used >= limit) {
            revert RateLimitExceeded(limit, uint256(bucket.used) + 1);
        }
    ...
    function encode(uint64 maxRequests_, uint64 windowSeconds_) internal pure returns (uint128) {
