...
/// @notice Canonical ABI codec for ASC bridge payload bytes.
library BridgeMessageCodecV1 {
    ...
    function encode(BridgePayloadV1 memory payload) internal pure returns (bytes memory) {
        return abi.encode(payload);
    }

    function decode(bytes memory encodedPayload) internal pure returns (BridgePayloadV1 memory) {
        return abi.decode(encodedPayload, (BridgePayloadV1));
    }

    function hasTokenTransfer(BridgePayloadV1 memory payload) internal pure returns (bool) {
        ASCBridgeTypes.EVMTokenAmount memory tokenAmount = payload.message.tokenAmount;
    ...
    function hasExecutionPayload(BridgePayloadV1 memory payload) internal pure returns (bool) {
    ...
    function decodeEvmReceiver(BridgePayloadV1 memory payload)
        internal
        pure
