...
contract BridgeIntentTemplateRegistry is Ownable {
    ...
    function tryExtract(string calldata name, bytes calldata data)
        external
        view
        returns (ExtractionResult memory result)
    {
    ...
    function tryExtractFirst(string[] calldata names, bytes calldata data)
        external
        view
        returns (string memory matchedName, ExtractionResult memory result)
    {
    ...
    function sliceCalldata(bytes calldata data, uint256 start)
        public
        pure
    ...
    function _extract(ParsedTemplate memory template_, bytes calldata data)
        internal
        view
        returns (ExtractionResult memory result)
    {
        ...
        if (template_.bodyKind == BodyKind.OpenForOrder) {
            (CrossChainOrderTypes.CrossChainOrder memory order,,) =
                abi.decode(bodyBytes, (CrossChainOrderTypes.CrossChainOrder, bytes, bytes));
            result.encodedBody = abi.encode(order);
            _fillIntentFields(template_, order.orderData, result);
            result.ok = true;
            return result;
    ...
    function _applyRuleKey(ParsedTemplate memory parsed, string memory key, string memory value)
        internal
        pure
        returns (ParsedTemplate memory)
    {
    ...
    function _parseSelector(string memory hexValue) internal pure returns (bytes4 selector) {
        ...
        selector = bytes4(
            (uint32(_hexNibble(raw[2])) << 28) | (uint32(_hexNibble(raw[3])) << 24)
                | (uint32(_hexNibble(raw[4])) << 20) | (uint32(_hexNibble(raw[5])) << 16)
                | (uint32(_hexNibble(raw[6])) << 12) | (uint32(_hexNibble(raw[7])) << 8)
                | (uint32(_hexNibble(raw[8])) << 4) | uint32(_hexNibble(raw[9]))
        );
    }
