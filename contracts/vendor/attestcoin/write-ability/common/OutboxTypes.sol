...
library OutboxTypes {
    ...
    function computeMessageId(address outbox, address emitter, uint64 sequence, bytes32 payloadHash)
        internal
        pure
        returns (bytes32)
    {
        return keccak256(abi.encode(outbox, emitter, sequence, payloadHash));
    }
}
