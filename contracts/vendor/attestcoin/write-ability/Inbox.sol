...
contract Inbox is IInbox, Ownable2Step, Pausable {
    ...
    function processedAt(bytes32 messageId) external view override returns (uint256) {
        return _processedAt[messageId];
    }

    function isPending(bytes32 messageId) external view override returns (bool) {
        return _isPending[messageId];
    }

    function validatedMessages(bytes32 messageId) external view returns (bool) {
        return _validatedMessages[messageId];
    }

    function validationFailed(bytes32) external pure returns (bool) {
    ...
    function retryPendingMessage(bytes32 messageId) external override whenNotPaused {
        ...
        bool delivered = _deliver(messageId, pending.emitterAddress, pending.payloadData);
    ...
    function setDefaultVoteValidator(IVoteValidator newValidator) external onlyOwner {
    ...
    function _deliver(bytes32 messageId, address emitterAddress, bytes memory payloadData)
        private
        returns (bool)
    {
    ...
    function _storePending(bytes32 messageId, address emitterAddress, bytes memory payload)
        private
    {
        _isPending[messageId] = true;
        _pendingMessages[messageId] =
            PendingMessage({emitterAddress: emitterAddress, payloadData: payload});
    }
}
