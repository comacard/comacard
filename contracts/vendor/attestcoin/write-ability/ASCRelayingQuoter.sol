...
contract ASCRelayingQuoter is IASCRelayingQuoter, Ownable2Step {
    ...
    event AcknowledgmentFeeUpdated(
        uint16 indexed chainId, uint256 indexed newAcknowledgmentFeeInAttest
    );
    event AttestCtcPoolPathUpdated(
        address attestToken, address ctcToken, address[] path, uint32 poolTwapWindow
    );
    ...
    constructor(address initialOwner, address twapReader_, address oracleService_)
    ...
    function requestQuote(
        ...
        view
        override
        returns (uint256 requiredPaymentInCTC)
    {
        return _quote(dstChain, gasLimit);
    }

    function requestQuoteInAttest(uint16 dstChain, address, bytes32, uint256 gasLimit)
        external
        view
        override
        returns (uint256 requiredPaymentInATTEST)
    {
        ...
        requiredPaymentInATTEST = FullMath.mulDiv(requiredPaymentInCTC, 1e18, ctcPerAttest);
    ...
    function requestExecutionQuote(
        ...
        emit ExecutionQuoteRequested(
            dstChain, targetContract, payloadHash, gasLimit, requiredPaymentInCTC
        );
    }
    ...
    function setPricingMode(PricingMode newMode, uint64 newSourcePrice)
        external
        override
        onlyOracle
    {
    ...
    function getAttestPerNative(uint16 dstChain)
        external
        view
        override
        returns (uint256 attestPerNative)
    {
        uint256 ctcPerAttest = _ctcPerAttest();
        if (ctcPerAttest == 0) revert RelayerErrors.InvalidPoolPrice();
    ...
    function _composePoolPath(address[] memory path, address tokenIn, address expectedOut)
        internal
        view
    ...
    function priceUpdate(uint64 newSourcePrice, uint16 chainId, PricingData calldata price)
        external
        override
        onlyOracle
    {
    ...
    function batchPriceUpdate(
        ...
        if (newSourcePrice == 0) {
            revert RelayerErrors.SourcePriceNotSet();
        }
    ...
    function getAcknowledgmentFee(uint16 dstChain)
        external
        view
        override
        returns (uint256 acknowledgmentFeeATTEST)
    {
    ...
    function setAcknowledgmentFee(uint16 dstChain, uint256 newAcknowledgmentFeeInAttest)
        external
        override
        onlyOracle
    {
    ...
    function setAttestCtcPool(
        ...
        if (attestToken_ == address(0) || ctcToken_ == address(0)) {
            revert CommonErrors.ZeroAddress();
        }
    ...
    function setNativeCtcPool(uint16 dstChain, address nativeToken_, address[] calldata path)
        external
        onlyOwner
    {
    ...
    function _quote(uint16 dstChain, uint256 gasLimit) internal view returns (uint256 feeInCTC) {
        ...
        feeInCTC = (p.baseFee + gasCostInCTC) * (BPS_DENOMINATOR + p.priceBuffer) / BPS_DENOMINATOR;
    }
}
