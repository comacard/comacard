...
contract RelayerContractLite is IRelayerContractLite, RelayerFeeLedger, Ownable2Step {
    ...
    constructor(
        ...
        if (
            attestToken_ == address(0) || outbox_ == address(0) || proofVerifier_ == address(0)
    ...
    function setAuthorizedQuoter(address quoter, bool authorized) external override onlyOwner {
    ...
    ///      send no value.
    function publishAndCollectRelayerFee(bytes calldata payload, bytes calldata signedQuote)
        external
        payable
        override
    ...
    function collectRelayerFee(bytes32 messageId, bytes calldata signedQuote)
        external
        payable
        override
    {
    ...
    function topUpGasLimit(
        ...
        if (_feesInNative(messageId)) {
            ...
            attestToken.compatibleTransferFrom(
                msg.sender, address(_vaultFor(messageId)), additionalATTEST
            );
        }
    ...
    function _depositRelay(bytes32 messageId, address payer, RelayerTypes.Quote memory q) internal {
