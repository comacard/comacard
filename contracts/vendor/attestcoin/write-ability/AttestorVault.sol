...
contract AttestorVault is IAttestorVault, Ownable2Step {
    ...
    constructor(
        ...
        if (
            attestToken_ == address(0) || outbox_ == address(0) || relayerContract_ == address(0)
                || validationContract_ == address(0) || attestorRegistry_ == address(0)
    ...
    function deposit(bytes32 messageId, address payer, uint32 targetChainId, uint256 coreFee)
        external
        override
        onlyDepositor
    {
    ...
    function depositWithAuthorization(
        ...
        IERC3009(address(attestToken))
            .receiveWithAuthorization(
                payer, address(this), coreFee, validAfter, validBefore, nonce, v, r, s
            );
    ...
    function settle(bytes32 messageId, uint32 chainId, address[] calldata validAttestors)
        external
        override
        onlyValidationContract
    {
