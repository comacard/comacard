...
contract EOAValidator is IVoteValidator, Ownable2Step {
    ...
    constructor(
        ...
        if (attestorRegistry_ == address(0)) {
            revert InvalidArgument("registry");
        }
    ...
    function calculateRequiredVotes(uint256 totalAttestors) public view returns (uint256) {
        uint256 t =
            ((totalAttestors * thresholdNumerator) / THRESHOLD_DENOMINATOR) + thresholdAddition;
    ...
    function updateAttestorSet(address[] calldata newAttestors) external onlyOwner {
        _validateQuorumConfig(
            newAttestors.length, minAttestorCount, thresholdNumerator, thresholdAddition
        );
    ...
    function updateThreshold(uint256 _numerator, uint256 _addition) external onlyOwner {
        _validateThresholdFraction(_numerator);
        _validateQuorumConfig(
            attestorRegistry.getAttestorCount(), minAttestorCount, _numerator, _addition
        );
    ...
    function updateMinAttestorCount(uint256 _minAttestorCount) external onlyOwner {
        _validateQuorumConfig(
        ...
        external
    {
        ...
        uint256 required = calculateRequiredVotes(attestorRegistry.getAttestorCount());
        if (unique < required) revert ThresholdNotMet(unique, required);
    ...
    function _validateQuorumConfig(
        ...
        unchecked {
            uint256 required = ((totalAttestors * numerator) / THRESHOLD_DENOMINATOR) + addition;
