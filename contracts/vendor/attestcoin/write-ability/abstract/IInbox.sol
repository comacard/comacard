...
interface IInbox {
    ...
    /// @notice Returns the Inbox message dispatcher (`IMessageReceiver` for outbound delivery)
    /// @return receiver The message dispatcher contract address
    function messageDispatcher() external view returns (address);

    /// @notice Updates the message dispatcher contract address
    /// @param receiver The new message dispatcher contract address
    function setMessageDispatcher(address receiver) external;
    ...
    /// @param relayer The caller who submitted this successful delivery
    /// @dev Only emitted when target contract execution succeeds
    event MessageDelivered(
        bytes32 indexed messageId, address indexed processor, address indexed relayer
    );
    ...
    /// @param relayer The caller who submitted this delivery attempt
    ...
    /// @dev relayer identifies the delivery-fee payee when this outcome is proven on the
    ///      source chain — RelayerFeeVault pays the proven relayer, not the claim caller
    /// @dev Anyone can retry pending messages using retryPendingMessage()
    event MessagePending(
        bytes32 indexed messageId, address indexed destinationContract, address indexed relayer
    );
    ...
    event DefaultVoteValidatorSet(address indexed oldValidator, address indexed newValidator);
}
