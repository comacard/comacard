...
interface IOutbox {
    ...
    event ForwarderApprovalSet(address indexed emitter, address indexed forwarder, bool approved);
    ...
    function getMessage(bytes32 messageId) external view returns (OutboxTypes.Message memory);
    ...
    function isForwarderApproved(address emitter, address forwarder) external view returns (bool);
    ...
    function isAcknowledged(bytes32 messageId) external view returns (bool acknowledged);

    /// @notice Returns whether a message was published with canAck = true
    /// @param messageId The message identifier
    /// @return canAck True if this message is tracked in storage
    function messageCanAck(bytes32 messageId) external view returns (bool canAck);
