...
contract Outbox is IOutbox, Storage, Ownable2Step, Pausable {
    ...
    constructor(
        ...
        if (
            initialValidator == address(0) || initialAttestorVault == address(0)
                || initialFeeRegistry == address(0) || initialAttestToken == address(0)
    ...
    function getState() external view override returns (OutboxTypes.StateView memory view_) {
        OutboxState storage s = _state();
    ...
    function getMessage(bytes32 messageId)
        ...
        view
        override
        returns (OutboxTypes.Message memory m)
    {
    ...
    function isForwarderApproved(address emitter, address forwarder)
        external
        view
        override
        returns (bool)
        ...
        returns (bytes32 messageId)
    {
        ...
        if (fee > 0) {
            IERC20(s.attestToken).compatibleTransferFrom(msg.sender, s.attestorVault, fee);
    ...
    function publishMessageWithAuthorization(
        ...
        if (fee > 0) {
            IAttestorVault(s.attestorVault)
    ...
    function publishMessageFrom(address emitter, uint256 ackFee, bytes calldata payload)
        external
        override
    ...
    function routeCoreFee(bytes32 messageId, address payer, uint256 coreFeeAmount)
        external
        override
        onlyTrustedForwarder
    {
    ...
    function routeAckFee(bytes32 messageId, address payer, uint256 ackFee)
        external
        override
        onlyTrustedForwarder
    {
    ...
    function batchAcknowledgeMessages(bytes32[] calldata messageIds)
        external
        override
        onlyValidator
    {
