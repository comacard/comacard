 ...
 * test the wrong app: a hand-run dev server on 3000 pops the real wallet modal and waits for a
 * signature nobody is there to give. Always start our own; `reuseExistingServer: false` plus a
 * private port keep the two from colliding.
 */
const PORT = 3100;
const BASE_URL = `http://localhost:${PORT}`;

/**
 * The app is mobile-first, so one project at a phone viewport is the honest default. The wallet is
 * stubbed at the `lib/wallet.ts` seam under NEXT_PUBLIC_E2E, so nothing here waits on an extension.
 */
